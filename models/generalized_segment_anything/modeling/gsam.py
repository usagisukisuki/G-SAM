...
class GSAM(nn.Module):
    ...
    def __init__(
        ...
        for n, value in self.image_encoder.named_parameters():
          if "cnn_embed" not in n and "post_embed" not in n and "Adapter" not in n and "2.attn.rel_pos" not in n and "5.attn.rel_pos" not in n and "8.attn.rel_pos" not in n and "11.attn.rel_pos" not in n and "upneck" not in n:
    ...
    def forward(
        ...
        get_dense_pe = self.positional_encoder(imge)
        de = de.expand(imge.shape[0], -1, imge.shape[2], imge.shape[3])
